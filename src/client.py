...
class Client:
    ...
    def __READ__(self):
        if (
            self.cache_.__data_exist__(
                self.inputs_[0], self.inputs_[1], self.inputs_[2]
            )
            == True
        ):
            self.__from_cache__(self.inputs_[0], SERV_OP[self.request_])
            self.cache_.__READ__(self.inputs_[0], self.inputs_[1], self.inputs_[2])
            self.status_ = 1
        else:
    ...
    '''
    This operation allows the users to list
    all the contents in cache.
    '''
    def __CACHE__(self):
        self.cache_.__LS__()
        self.status_ = 1
    ...
    '''
    This operation allows the users to list
    all the contents in cache in details.
    '''
    ...
    '''
    This operation displays current time.
    '''
    def __TIME__(self):
    ...
    '''
    This operation displays help table.
    '''
    def __HELP__(self):
        self.__help__()
        self.status_ = 1
    ...
    def __from_cache__(self, filename, command):
        c1 = 'The file "%s" exists in Cache!' % (filename)
        c2 = '%s "%s" from Cache...' % (command, filename)
        print("\n" + c1 + "\n" + c2 + "\n")
    ...
    It either returns an integer or quits the program.
    '''
    ...
    def __help__(self):
        print(
            ...
            + "read(r)"
            + "\033[0m"
            + " (\033[4mfilename\033[0m, \033[4moffset(int)\033[0m, \033[4mbytes-to-read(int)\033[0m"
        )
        ...
        print(
            ...
            + "write(w)"
            + "\033[0m"
            + " (\033[4mfilename\033[0m, \033[4moffset(int)\033[0m, \033[4mContents\033[0m)"
        )
        ...
        print(
            ...
            + "monitor(m)"
            + "\033[0m"
            + " (\033[4mfilename\033[0m, \033[4mDuration(int)\033[0m)"
        )
        print(
            ' : Monitor the change / update in file "filename" for "Duration" amount of time'
        )
        print("\n")
        ...
        print(
            ...
            + "rename(x)"
            + "\033[0m"
            + " (\033[4mfilename\033[0m, \033[4mnew-file-name\033[0m)"
        )
        ...
        print(
            ...
            + "create(n)"
            + "\033[0m"
            + " (\033[4mfilename\033[0m, \033[4mContents\033[0m)"
        )
        ...
        print(
            ...
            + "erase(e)"
            + "\033[0m"
            + " (\033[4mfilename\033[0m, \033[4moffset(int)\033[0m, \033[4mbytes-to-erase(int)\033[0m)"
        )
        print(
            ' : Erase the contents on file "filename" from offset to "bytes to erase"'
        )
        print("\n")
...
if __name__ == "__main__":
    ...
    client.__process_args__(client.__get_args__())
