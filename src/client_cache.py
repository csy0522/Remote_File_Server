...
class Client_Cache:
    ...
    '''
    This operation allows the client to see
    all the existing files in cache in details.
    '''
    def __LSL__(self):
        print("\nList all files in Cache")
        print("\n================= CACHE =================")
    ...
    def __add__(self, key, content, offset, b2r, last_index):
        self.storage_[key] = [
            ...
            b2r,
            last_index,
            CUR + timedelta(seconds=self.time_out_),
        ]
            ...
            if self.client_req_ == "RENAME":
                if self.one_copy_semantics_[0] != self.one_copy_semantics_[1]:
                    ...
                    self.__update_history__(
                        self.client_req_, self.one_copy_semantics_[0], success=True
                    )
                    self.__update_log__(
                        self.one_copy_semantics_[0],
                        "Modified from %s Operation" % (self.client_req_),
                    )
            else:
                last_index_ = len(self.one_copy_semantics_[1])
                if ori_content_ != self.one_copy_semantics_[1][offset_:b2r_]:
                    print(
                        "File %s updated in Cache through One_Copy_Semantics"
                        % (self.one_copy_semantics_[0])
                    )
