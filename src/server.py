...
class Server:
    ...
    def __READ__(self):
        ...
        if self.__file_exist__(
            ...
            self.__record__(self.client_[0],str(self.port_),
                self.__dec__(self.client_req_,'str'),
                self.__dec__(filename,'str'),success=succ)
            ...
            self.__record__(self.client_[0],str(self.port_),
                self.__dec__(self.client_req_,'str'),
                self.__dec__(filename,'str'),success=False)
        print("----------------------- END -----------------------")
    ...
    def __MONITOR__(self):
        ...
        if self.__file_exist__(
            ...
            ori_file = open(self.serv_dir_+self.__dec__(filename, 'str'))
            ori_content = ori_file.read()

            while now < des:
                now = datetime.now()
                new_file = open(self.serv_dir_+self.__dec__(filename, 'str'))
